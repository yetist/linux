				    ...
				    struct hw_perf_event *hwc)
{
	...
	for (i = LOONGARCH_MAX_HWEVENTS - 1; i >= 0; i--) {
...
static void loongarch_pmu_enable_event(struct hw_perf_event *evt, int idx)
{
	...
	WARN_ON(idx < 0 || idx >= LOONGARCH_MAX_HWEVENTS);
...
static irqreturn_t pmu_handle_irq(int irq, void *dev)
{
	...
	for (n = 0; n < LOONGARCH_MAX_HWEVENTS; n++) {
...
{
	unsigned long flags;
	int ctr = LOONGARCH_MAX_HWEVENTS;
	struct cpu_hw_events *cpuc = this_cpu_ptr(&cpu_hw_events);
...
static int __init init_hw_perf_events(void)
{
	...
	pr_cont("%s PMU enabled, %d %d-bit counters available to each CPU.\n",
			loongarch_pmu.name, LOONGARCH_MAX_HWEVENTS, 64);
