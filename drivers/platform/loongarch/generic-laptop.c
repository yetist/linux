...
static int hotkey_map(void)
{
	...
	status = acpi_evaluate_object_typed(hotkey_handle, METHOD_NAME__KMAP, NULL, &buf, ACPI_TYPE_PACKAGE);
	if (status != AE_OK) {
...
static struct generic_sub_driver generic_sub_drivers[] __initdata = {
	{
		.name = "EC Event",
		.init = event_init,
		.notify = event_notify,
