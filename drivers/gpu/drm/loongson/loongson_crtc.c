					 ...
					 struct drm_atomic_state *state)
{
	...
	if (crtc_read(lcrtc, FB_CFG_DVO_REG) & CFG_FBNUM)
		crtc_write(lcrtc, FB_ADDR0_DVO_REG, drm_fb_cma_get_gem_addr(pstate->fb, pstate, 0));
	else
		crtc_write(lcrtc, FB_ADDR1_DVO_REG, drm_fb_cma_get_gem_addr(pstate->fb, pstate, 0));
